...
Тип byte
Размер допустимых значений типа byte не так уж велик: от -128 до +127. 

Вещественные типы

Тип float

Размер этого типа совсем небольшой — всего 4 байта (32 бита), но он может хранить значения от -3.4*1038 до 3.4*1038.
...
Тип char в Java предназначен для хранения символов и представляет собой примитивный тип, который также можно интерпретировать как числовой. Он хранит коды символов из кодировки Unicode, где каждому символу соответствует числовой код. Переменная типа char занимает два байта памяти и может хранить значения от 0 до 65,535, будучи беззнаковым типом. Таким образом, char является гибридным типом, позволяющим работать с символами как с числами, что упрощает обработку данных.
